*** Settings ***
Library           ../../../resources/keywords/BB-TRF-WWAN-002.py
...
WiFi LAN Performance Test (Dynamic)
    [Documentation]    Full Wi-Fi performance validation per BB-TRF-WWAN-002
    [Tags]             BB-TRF-WWAN-002

    Configure WiFi Devices
    Set Iperf Port Range        ${START_PORT}    ${END_PORT}
    Run Full Mesh TCP Test
    Capture Ifconfig Snapshot For Devices    ${ALL_DEVICES}    ${INTERFACE}
    Run Full Mesh UDP Test
    Capture Ifconfig Snapshot For Devices    ${ALL_DEVICES}    ${INTERFACE}
    Validate Ifconfig Errors For Devices     ${ALL_DEVICES}    ${INTERFACE}
    Ping All Devices Using Interface         ${PC_LIST}    ${INTERFACE}

    Log To Console               ==== Completed WiFi LAN Performance Test ====
    Log Message To Custom File   ==== Completed WiFi LAN Performance Test ====
...
Load Device Info
    ...
    # ================= TEST PARAMS =================
    ${SSID}=        Set Variable    ${test_config["SSID"]}
    ${PSK}=         Set Variable    ${test_config["PSK"]}
    ${ENCRYPTION}=  Set Variable    ${test_config["encryption"]}
...
Configure WiFi Devices
    Log Message To Custom File    Configuring OpenWrt AP and RPi WiFi clients
    Configure Devices Parallel    ${DUT}    ${PC_LIST}    ${SSID}    ${PSK}


Run Full Mesh TCP Test
    Log Message To Custom File    Running full-mesh TCP iperf3 test
