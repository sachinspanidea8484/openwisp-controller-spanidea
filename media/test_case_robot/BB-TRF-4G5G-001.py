# START_DESCRIPTION 
#1. Initialize logging and establish connections to DUT & RPi's
#2. Check the DUT connection, wwan0 interface and default route.
#3. Baseline connectivity check via ping & collect the latency.
#4. Print baseline stats on DUT (CPU, memory used, interrupts).
#5. Check RPi routing and connectivity
#6. Keep printing baseline stats on DUT while test is running (CPU, memory used, interrupts)
#7. Collect logs/results
#8. Print baseline stats on DUT after test completion (CPU, memory used, interrupts).
# END_DESCRIPTION
...
def initialize_custom_log_file():
    log_message("==== Custom log initialized ====")
...
def check_rpi_routing_dynamic(device_name, preferred_iface=None):
    ...
    log_message(f"{device_name}: Using interface {iface}")
    ...
    if match:
        ...
        log_message(f"{device_name}: Added default route via {gw} dev {iface}")
        return True
...
def ping_all_rpis(pc_name, iface, remote_ip, duration=10):
    ...
    log_message(f"Pinging {remote_ip} from {pc_name} via {iface}")
