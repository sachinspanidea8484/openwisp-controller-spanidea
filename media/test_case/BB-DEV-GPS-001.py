...
MAX_LINES = 5   # Number of GPS readings to collect

def log(message):
    timestamp = datetime.now().strftime("[%Y-%m-%d %H:%M:%S]")
    print(f"{timestamp} {message}")

def run_command_with_read_limit(command, max_lines=MAX_LINES, max_time=10):
...
def verify_gps_output(lines):                                                                            
    """Verify GPS readings: collected & not identical."""               
    readings = []                                                                      
    for line in lines:                                                  
    ...
    if len(readings) < MAX_LINES:                                       
        log(f"[FAIL] Could not collect {MAX_LINES} valid GPS readings.")                            
        return False                                                                                     
                                                                                                    
    # Check for identical readings                                                                       
    if all(r == readings[0] for r in readings):                                                     
        log(f"[FAIL] All {MAX_LINES} GPS readings are identical. Possible hardcoded/static values.")
        return False                                                                                     
...
def main():                                                                                         
    ...
    output_lines, error_lines = run_command_with_read_limit(GPS_SCRIPT, max_lines=MAX_LINES, max_time=10)
