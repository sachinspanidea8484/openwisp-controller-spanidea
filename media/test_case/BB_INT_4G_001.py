...
#2. Attach the DUT to the 4G network and verify.
...
#!/usr/bin/env python3
"""
4G WAN INTERFACE TEST (BB-INT-4G-001)
Dynamic target band input via command-line parameter
python3 BB_INT_4G_001.py CONFIGURATION='{"cellular_interface": "Modem1", "remote_ping_ip": "8.8.8.8", "test_duration" : "60", "error_threshold_percent":"10"}'
"""
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='4G WAN INTERFACE (BB-INT-4G-001)')
    ...
    try:
        ...
        if NETWORK_TYPE != "4G":
            log("TEST FAILED - %s is not a 4G WAN interface but has %s",
