...
def update_test_status_api(execution_id, status, exit_code=0, stdout="", stderr="", started_at=None, completed_at=None, device_rebooting=False):
    ...
    # ✅ Only add completed_at for final statuses
...
def download_helper_files(helpers):
    ...
    for helper_name in helpers:
        ...
        try:
            ...
            if result.returncode != 0:
                log_message(f"⚠️ Warning: Failed to download {helper_name}: {result.stderr}")
                continue
            ...
            if os.path.exists(helper_path):
                ...
                log_message(f"⚠️ Warning: {helper_name} not found after download")
                
        except Exception as e:
            log_message(f"⚠️ Warning: Error downloading {helper_name}: {str(e)}")
...
def execute_test(test_id, execution_id ,test_json_str):
    ...
    try:
        ...
        if is_file_required and file_download_url:
            ...
            log_message(f"\n⏭️ Step 5.5: No configuration file required")
