...
class NotificationListSerializer(NotificationSerializer):
    ...
    def get_target_url(self, obj):
        ...
        # Case 1: nested data (your current case)
        nested = obj.data.get("data")
        if isinstance(nested, dict) and nested.get("target_url"):
            return nested["target_url"]

        # Case 2: flat data (future-safe)
        if obj.data.get("target_url"):
            return obj.data["target_url"]

        # Case 3: fallback to target object
        if obj.target:
            try:
